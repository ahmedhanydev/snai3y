...
import { Star, MapPin, Phone, Mail, Search, Wrench, Filter } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function Craftsmen() {
  ...
  const filteredCraftsmen = craftsmen.filter(craftsman => {
    const matchesSearch = 
      ...
      craftsman.specialization.toLowerCase().includes(searchQuery.toLowerCase()) ||
      craftsman.location.toLowerCase().includes(searchQuery.toLowerCase());
    
    const matchesSpecialization = 
      specializationFilter === 'all' || craftsman.specialization === specializationFilter;
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-8">
          ...
          <Select 
            ...
            onValueChange={(value: any) => setAvailabilityFilter(value as 'all' | 'available' | 'busy')}
          >
            <SelectTrigger>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCraftsmen.map((craftsman) => (
            <Card key={craftsman.id} className="p-6 hover:shadow-lg transition-all duration-300">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <Avatar className="w-16 h-16">
                    <AvatarImage src="" />
                    <AvatarFallback className="text-xl">{craftsman.name[0]}</AvatarFallback>
                  </Avatar>
                  <div>
                    <h3 className="font-semibold text-lg">{craftsman.name}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      <Wrench className="w-4 h-4 text-gray-500" />
                      <span className="text-gray-600">{craftsman.specialization}</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">{craftsman.completedJobs} مهمة مكتملة</Badge>
                </div>
              ...
              <div className="border-t pt-4">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="font-semibold text-lg">{craftsman.hourlyRate} جنيه</span>
                </div>
        ...
        {filteredCraftsmen.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500">لا يوجد صنايعية مطابقين لمعايير البحث</p>
          </div>
        )}
      </div>
    </div>
